...
import { Trophy, Star, Zap, ShoppingBag } from 'lucide-react';

export function PointsForm() {
  const { classes, students, addPoint, redeemPoints, getStudentStats } = useData();
  ...
  const handleAward = (e: React.FormEvent) => {
    ...
    addPoint(selectedStudentId, parseInt(amount), reason, type);
    toast.success('积分已颁发！');
    setReason('');
  };
      ...
      <div className="p-8 bg-white rounded-xl border border-slate-200 shadow-sm transition-all duration-300">
        <div className="space-y-6">
          ...
          {selectedStudentId && currentStudentStats && (
            <div className={`text-center p-3 rounded-lg border ${mode === 'award' ? 'bg-blue-50 border-blue-100 text-brand-blue' : 'bg-orange-50 border-orange-100 text-orange-600'}`}>
